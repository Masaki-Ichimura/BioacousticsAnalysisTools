...
class DOA(torch.nn.Module):
    def __init__(
        ...
        if freq_bins is not None:
            freq_bins = np.array(freq_bins, dtype=np.int)
        elif freq_hz is not None:
            freq_bins = [int(np.round(f / sample_rate * n_fft)) for f in freq_hz]
        else:
            freq_range = [int(np.round(f / sample_rate * n_fft)) for f in freq_range]
            freq_bins = np.arange(*freq_range, dtype=np.int)
    ...
    def precompute_mode_vec(self):
        ...
        r_x = self.mic_locs[0, None, :, None]
        r_y = self.mic_locs[1, None, :, None]

        if self.mic_locs.shape[0] == 3:
            r_z = self.mic_locs[2, None, :, None]
        else:
